  ...
  try{
   ...
   stage('SERVICE - Git checkout'){
    ...
    stage('SERVICE - Tests unitaires'){
      sh 'docker run --rm --name maven-${commitIdLong} -v /var/lib/jenkins/maven/:/root/.m2 -v "$(pwd)":/usr/src/mymaven --network generator_generator -w /usr/src/mymaven maven:3.3-jdk-8 cd spring-boot-server & mvn -B clean test'
    }

    /* Maven - build */
    stage('SERVICE - Jar'){
      sh 'docker run --rm --name maven${commitIdLong} -v /var/lib/jenkins/maven/:/root/.m2 -v "$(pwd)":/usr/src/mymaven --network generator_generator -w /usr/src/mymaven maven:3.3-jdk-8 cd spring-boot-server & mvn -B clean install'
    }
